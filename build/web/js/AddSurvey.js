...
function addFreeAnswer(count) {
    var freeAnswer = '<div class="row margin-button-20 free-answer-textarea' + count + '">' +
            ...
            '<input required="true" type="text" class="form-control freeAnswer" name="free-answer-question-' + count + '" aria-label="Text input with dropdown button" id="free-answer-question' + count + '"' + ' placeholder="Write Your Question">' +
...
function mcqHTMLBody(mcqCounter)
{
    var mcq = '<div class="row margin-button-20 mcq-parent mcq-parent-number-' + mcqCounter + '">' +
            ...
            '<div class="input-group">' +
            '<input required="true" type="text" name="mcq-question-' + mcqCounter + '" placeholder="Write your Question Here" class="form-control mcq"  aria-label="Text input with dropdown button">' +
            '<div class="input-group-btn">' +
...
function addRadioButton(id)
{
    ...
    var answer = '<div class="col-lg-10 margin-3 mcq-answer-number-' + id + '">' +
            ...
            '</span>' +
            '<input required="true" type="text" name="mcq-number-' + id + '-' + subAnswerID + '" placeholder="Write Answer Here" class="form-control" aria-label="Text input with radio button">' +
...
function checkBoxHTMLBody(checkboxCounter)
{
    var mcq = '<div class="row margin-button-20 checkbox-parent check-box-question checkbox-parent-number-' + checkboxCounter + '">' +
            ...
            '<div class="input-group">' +
            '<input required="true" type="text" name="checkbox-question-' + checkboxCounter + '" placeholder="Write your Question Here" class="form-control checkbox"  aria-label="Text input with dropdown button">' +
            '<div class="input-group-btn">' +
...
function addCheckBox(id)
{
    ...
    var answer = '<div class="col-lg-10 check-box-answer margin-3 checkbox-answer-number-' + id + '">' +
            ...
            '</span>' +
            '<input required="true" type="text" name="checkbox-answer-' + id + '-' + subAnswerID + '" placeholder="Write Answer Here" class="form-control" aria-label="Text input with radio button">' +
            '<span class="input-group-btn">' +
...
//    e.preventDefault();
    ...
    if ($("input[type='text']").length < 5) {
        $(".add-survey-alert").text("can't  submit empty survey");
        $(".add-survey-alert").removeAttr("hidden");
        e.preventDefault();
    }else
    {
        $(".add-survey-alert").attr("hidden","true");
    }
    
    if ($(".mcq").length > 0) {
        if ($("input[type='radio']").length < $(".mcq").length * 2) {
            $(".add-survey-alert").text("each radio button must have");
            $(".add-survey-alert").removeAttr("hidden");
            e.preventDefault();
        }else
        {
    ...
    if ($(".checkbox").length > 0) {
        if ($("input[type='checkbox']").length < $(".checkbox").length * 2) {
            $(".add-survey-alert").text("each checkbox button must have");
            $(".add-survey-alert").removeAttr("hidden");
            e.preventDefault();
        }else
        {
